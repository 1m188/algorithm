...
#
#
# k 表示到当前位置为止的连续 "prev" 字符串数目（包含当前字符串），令下标从 0 开始的 整数 数组 nums 表示目前为止遍历过的所有整数，同时用
# nums_reverse 表示 nums 反转得到的数组，那么当前 "prev" 对应的 上一个遍历的整数 是 nums_reverse 数组中下标为
# (k - 1) 的整数。
    ...
    def lastVisitedIntegers(self, words: List[str]) -> List[int]:
        ...
        for word in words:
            if word.isdigit():
                ...
                k += 1
                if k > len(nums): res.append(-1)
                else: res.append(nums[-k])
        return res
