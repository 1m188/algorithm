/*
 * @lc app=leetcode.cn id=100248 lang=csharp
 *
...
public class Solution
{
    ...
    {
        var flags = new bool[26, 26];
        for (int i = 0; i < s.Length - 1; i++)
        {
            flags[(int)(s[i]) - (int)'a', (int)(s[i + 1]) - (int)'a'] = true;
        }

        for (int i = s.Length - 1; i > 0; i--)
        {
            if (flags[(int)(s[i]) - (int)'a', (int)(s[i - 1]) - (int)'a']) return true;
        }
        return false;
    }
}
