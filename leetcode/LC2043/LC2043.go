...
func Constructor(balance []int64) Bank {
	bank := Bank{Acount: make([]int64, len(balance)+1)}
	copy(bank.Acount[1:], balance)
	return bank
}

func (this *Bank) Transfer(account1 int, account2 int, money int64) bool {
	n := len(this.Acount)
	if !(account1 >= 1 && account1 <= n) || !(account2 >= 1 && account2 <= n) || this.Acount[account1] < money {
		return false
	}
	this.Acount[account1] -= money
	this.Acount[account2] += money
	return true
}
...
func (this *Bank) Deposit(account int, money int64) bool {
	...
	this.Acount[account] += money
	return true
}

func (this *Bank) Withdraw(account int, money int64) bool {
	n := len(this.Acount)
	if !(account >= 1 && account <= n) || this.Acount[account] < money {
		return false
	}
	this.Acount[account] -= money
	return true
