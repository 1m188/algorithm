...
class Solution:
    ...
    def categorizeBox(self, length: int, width: int, height: int,
                      mass: int) -> str:

        classes = []
        if (length >= 10**4 or width >= 10**4
                or height >= 10**4) or length * width * height >= 10**9:
            classes.append('Bulky')
        if mass >= 100: classes.append('Heavy')
        if len(classes) == 2: return 'Both'
        if len(classes) == 0: return 'Neither'
        return classes[0]
