...
class ATM:
    ...
    def withdraw(self, amount: int) -> List[int]:
        ...
        banknotesCount = self.banknotesCount.copy()

        res = [0, 0, 0, 0, 0]

        for money, index in [[500, 4], [200, 3], [100, 2], [50, 1], [20, 0]]:
            if amount >= money and banknotesCount[index] > 0:
                x = amount // money
                y = banknotesCount[index]
                if x > y:
                    amount -= y * money
                    res[index] += y
