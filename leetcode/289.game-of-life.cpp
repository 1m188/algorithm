...
public:
    void gameOfLife(vector<vector<int>> &board) {
        ...
        auto count = [&](int a, int b) -> int {
            int cnt = 0;
            if (a - 1 >= 0 && b - 1 >= 0) cnt += board[a - 1][b - 1];
            if (a - 1 >= 0) cnt += board[a - 1][b];
            if (a - 1 >= 0 && b + 1 < n) cnt += board[a - 1][b + 1];
            if (b - 1 >= 0) cnt += board[a][b - 1];
            if (b + 1 < n) cnt += board[a][b + 1];
            if (a + 1 < m && b - 1 >= 0) cnt += board[a + 1][b - 1];
            if (a + 1 < m) cnt += board[a + 1][b];
            if (a + 1 < m && b + 1 < n) cnt += board[a + 1][b + 1];
            return cnt;
        };
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (board[i][j]) {
                    if (counts[i][j] < 2 || counts[i][j] > 3) board[i][j] = 0;
