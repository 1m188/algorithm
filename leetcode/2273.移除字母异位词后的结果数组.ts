...
function removeAnagrams(words: string[]): string[] {
    ...
    function judge(s1: string, s2: string): boolean {
        let s1s = Array.from(s1.substring(0)), s2s = Array.from(s2.substring(0));
        s1s.sort();
        s2s.sort();
        return s1s.toString() == s2s.toString(); // 注意ts之中数组不能直接比较是否相等，要先转换成字符串
    }
