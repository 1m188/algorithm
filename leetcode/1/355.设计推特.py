...
# Testcase Example:  '["Twitter","postTweet","getNewsFeed","follow",
...
class Twitter:
    ...
    def postTweet(self, userId: int, tweetId: int) -> None:
        ...
        Compose a new tweet.
        """
        self.tl.append(self.Tweet(userId, tweetId))

    def getNewsFeed(self, userId: int) -> List[int]:
        """
        Retrieve the 10 most recent tweet ids in the user's news feed. Each \
            item in the news feed must be posted by users who the user \
            followed or by the user herself. Tweets must be ordered from \
            most recent to least recent.
        """
        self.fl.setdefault(userId, {userId})
        res = [
            i.tweetId for i in reversed(self.tl) if i.userId in self.fl[userId]
        ]
        return res[:10]
    ...
    def follow(self, followerId: int, followeeId: int) -> None:
        ...
        Follower follows a followee. If the operation is invalid, it should \
            be a no-op.
        """
        self.fl.setdefault(followerId, {followerId})
    ...
    def unfollow(self, followerId: int, followeeId: int) -> None:
        ...
        """
        self.fl.setdefault(followerId, {followerId})
        if followerId != followeeId and followeeId in self.fl[followerId]:
