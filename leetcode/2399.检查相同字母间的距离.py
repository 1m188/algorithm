...
class Solution:
    ...
    def checkDistances(self, s: str, distance: List[int]) -> bool:
        ...
        s_li: List[int] = [-1 for _ in range(26)]
        for i, v in enumerate(s):
            j = ord(v) - ord('a')
            if s_li[j] == -1: s_li[j] = i
            else: s_li[j] = i - s_li[j] - 1

        # for i, v in enumerate(distance):
        #     if s_li[i] == -1: continue
        #     if s_li[i] != v: return False

        for i, v in enumerate(s_li):
            if v == -1: continue
            if s_li[i] != distance[i]: return False
