...
def isIPLegal(ip: str) -> bool:
    ...
    判断ip是否合法
    '''
    ipit = map(int, ip.split('.'))
    for v in ipit:  # 这里迭代可以用迭代器
        if not (v >= 0 and v <= 255): return False
    return True
...
def isMaskLegal(mask: str) -> bool:
    ...
    for v in mask_li:
        ...
        while len(x) < 8:
            x = '0' + x
        mask_bin += x
    if mask_bin[0] == '0' or mask_bin[-1] == '1': return False
    return '01' not in mask_bin


def judge(ip1: str, ip2: str, mask: str) -> bool:
    '''
    判断两个ip是否在同一个子网下
    '''
    ip1li = list(map(int, ip1.split('.')))  # 这里后续用索引访问值不能直接迭代
    ip2li = list(map(int, ip2.split('.')))
    maskli = list(map(int, mask.split('.')))
    for i in range(4):
        if ip1li[i] & maskli[i] != ip2li[i] & maskli[i]: return False
    return True
...
if not isIPLegal(ip1) or not isIPLegal(ip2) or not isMaskLegal(mask): print(1)
elif judge(ip1, ip2, mask): print(0)
else: print(2)
