...
class Solution:
    ...
    def sortEvenOdd(self, nums: List[int]) -> List[int]:
        ...
        oushu = [nums[i] for i in range(len(nums)) if not i % 2]
        jishu = [nums[i] for i in range(len(nums)) if i % 2]

        jishu.sort(reverse=True)
        oushu.sort()

        ans = []
        while jishu and oushu:
            ans.append(oushu.pop(0))
            ans.append(jishu.pop(0))
        if jishu: ans.extend(jishu)
        if oushu: ans.extend(oushu)
