...
# 内存限制：512.0MB
# 问题描述：
# 题目背景# 西西艾弗岛景色优美，游人如织。但是，由于和外界的交通只能靠渡船，交通的不便严重制约了岛上旅游业的发展。西西艾弗岛管委会经过努力，争取到了一笔投资，建设了一个通用航空机场。在三年紧锣密鼓的主体建设后，西西艾弗岛通用航空机场终于开始进行航站楼内部软硬件系统的安装和调试工程了。小 C 是机场运营公司信息部的研发工程师，最近，信息部门的一项重要任务是，研发登机牌自助打印系统。如图所示的是设计部门根据国际民航组织的行业标准设计的登机牌样张
...
# 28	数字	数字	大写
# 29	填充	填充	填充
# 按照这个方法可以得到一系列的不超过 30 的数字。如果有奇数个这样的数字，则在最后补充一个 29，使之成为偶数个。将它们两两成组，假设
#  和
#  是一组中连续出现的两个数字，那么可以得到一个码字是：
...
# 214, 841, 821, 449
# Data
# 接下来要计算校验码。校验码字的数目，由校验级别确定。假设校验级别为
# ，则校验码字的数目为
# 。特别地，如果指定了
# ，则表示不需要计算校验码字。要计算校验码字，首先要确定数据码字。数据码字由以下数据按顺序拼接而成（如图所示）：
...
# Data
# 样例1解释
# 要求编码数据是 HELLO，首先查表将其对应成数字。注意，由于编码器在开始时就处于大写字母模式，因此不需要额外的模式切换。因此对应成的数字为：7, 4, 11, 11, 14。由于只有奇数个数字，因此补充 29，形成序列 7, 4, 11, 11, 14, 29。然后两两成组计算码字：
# ，以此类推，得到 214, 341, 449。本输入不要求产生校验码，且有效数据区的宽度是 5 码字。目前有效数据的码字是 3 个，加上开头要添加的长度码字，共有 4 个码字。因此，需要补充一个填充码字，使得总码字数达到 5 个，充满一行。注意，长度码字中的长度数据包括所有数据码字，因此长度码字是 5 而不是 4。最终可以得到码字序列 5, 214, 341, 449, 900。
...
# 测试系统显示运行错误，不知道哪里出错了，另外一个C++的同样代码逻辑的版本得了40分
# 没有计算校验码

w, s = map(int, input().split())
string: str = input()
seq: list[int] = []
mode = 0  # 编码模式。0：大写模式；1：小写模式；2：数字模式
...
if len(seq) % 2 == 1: seq.append(29)  # 填充

# 码字
code = [30 * seq[i] + seq[i + 1] for i in range(0, len(seq) - 1, 2)]

# 计算数据码字
k = 0 if s == -1 else 2**(s + 1)  # 校验码长度
code.insert(0, 0)
n = len(code)
while (n + k) % w != 0:
    code.append(900)
