...
public class _6 {
    public static void main(String[] args) throws Exception {
        ...
        while (T-- > 0) {
            // 解析去程和回程的当地时间差
            long t1 = parseTime(sc.nextLine());
            long t2 = parseTime(sc.nextLine());
            // 设时差为d，飞行时间为f
            // 去程: f = t1 - d => t1 = f + d
            // 回程: f = t2 + d => t2 = f - d
            // 两式相加: t1 + t2 = 2f => f = (t1 + t2) / 2
            long f = (t2 + t1) / 2;
            System.out.printf("%02d:%02d:%02d\n", f / 3600, (f % 3600) / 60, f % 60);
        }
    }
    ...
    static long parseTime(String line) {
        ...
        if (idx != -1) {
            dayOffset = line.charAt(idx + 2) - '0';
            line = line.substring(0, idx).trim();
        }
        String[] parts = line.split("\\s+");
        long depart = toSeconds(parts[0]);
        long arrive = toSeconds(parts[1]) + dayOffset * 86400L;
        return arrive - depart;
    }

    // 将 h:m:s 格式时间转换为秒数
    static long toSeconds(String s) {
        String[] p = s.split(":");
