...
class Main {
    ...
    static int bfs(String[][] grid, int n, int sr, int sc, int er, int ec) {
        ...
        // queue element: {row, col, prevType, steps}
        // prevType: 0=none (from A), 1=last was '+', 2=last was '-'
        ...
        while (!q.isEmpty()) {
            ...
            for (int[] d : dirs) {
                ...
                // Reached B?
                if (nr == er && nc == ec) {
                    // If prev is '+' or '-', can always step onto B
                    // If prev is 0 (from A directly to B), that's fine too
                    return steps + 1;
                }

                // From A (prev=0): can step onto '+' or '-'
                if (prev == 0) {
                    int nextPrev = cell.equals("+") ? 1 : 2;
                    if (!visited[nr][nc][nextPrev]) {
                        visited[nr][nc][nextPrev] = true;
                        q.add(new int[] { nr, nc, nextPrev, steps + 1 });
                    }
                }
                // From '+': must step onto '-'
                else if (prev == 1 && cell.equals("-")) {
                    if (!visited[nr][nc][2]) {
