...
/**
 * 题目要求分析：
 * printInBuf() 方法中，buf[y][p2] = '|' 绘制竖线，
 * buf[y+1] 行绘制横线、节点数字、以及左右子树的 '/' 和 '\'。
 * 
 * 第 70 行空白处需要将节点值 sv 的每个字符填入 buf[y+1] 行，
 * 起始列为 p2（节点数字的水平中心），逐个字符覆盖横线 '-'.
 * 
 * 因此填空代码为：buf[y+1][p2+i] = sv.charAt(i)
 *
 * 下方将完整内容复制到 _3 类中。
 */
public class _3 {
