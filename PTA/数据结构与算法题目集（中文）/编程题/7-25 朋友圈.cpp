...
/* 一开始用的图，制造一个邻接矩阵，然后对每个没有遍历的节点做bfs找连通块，连通块里节点最多
的部分就是最大的朋友圈，但是后来内存超限了。又用C++自带的set操作函数来做，对于每个俱乐部
看作一个集合，对于和已有集合能够有交集的则并入，如果没有则作为另一个独立的朋友圈存在，
然后找最终所有朋友圈里最大的，但是后来想着两个互相独立的朋友圈也有可能因为其他俱乐部里的人
的存在而连到一起，所以又放弃了。后来上网查了一下，发现人家都是用并查集做的。 */
